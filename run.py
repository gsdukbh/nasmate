...
def get_run_config():
  ...
  if app_conf:
    ...
    _web_port = int(app_conf.get('web_port')) if str(
      app_conf.get('web_port', '')).isdigit() else 3000
    _ssl_cert = app_conf.get('ssl_cert')
    _ssl_key = app_conf.get('ssl_key')
    _ssl_key = app_conf.get('ssl_key')
    _debug = True if app_conf.get("debug") else False

  app_arg = dict(host=_web_host, port=_web_port, debug=_debug, threaded=True,
                 use_reloader=False)
  if _ssl_cert:
...
def monitor_config():
  ...
  _observer.schedule(_ConfigHandler(), path=Config().get_config_path(),
                     recursive=False)
...
if __name__ == '__main__':
  ...
  if is_windows_exe:
    ...
    if len(os.popen("tasklist| findstr %s" % os.path.basename(sys.executable),
                    'r').read().splitlines()) <= 2:
