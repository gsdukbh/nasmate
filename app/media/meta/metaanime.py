...
class MetaAnime(MetaBase):
  ...
  def __init__(self,
      ...
      tmdb_id=None,
      imdb_id=None):
    super().__init__(title, subtitle, fileflag, filePath, media_type, cn_name,
                     en_name, tmdb_id, imdb_id)
    if not title:
      return
    # 调用第三方模块识别动漫
    try:
      ...
      if anitopy_info:
        ...
        if not name or name in self._anime_no_words or (
            len(name) < 5 and not StringUtils.is_chinese(name)):
          # 普遍上第一个非字幕组的 `[]` 中内容为标题
        ...
        if self.cn_name:
          _, self.cn_name, _, _, _, _ = StringUtils.get_keyword_from_string(
              self.cn_name)
          if self.cn_name:
            self.cn_name = re.sub(r'%s' % self._name_nostring_re, '',
                                  self.cn_name, flags=re.IGNORECASE).strip()
            self.cn_name = zhconv.convert(self.cn_name, "zh-hans")
        if self.en_name:
          self.en_name = re.sub(r'%s' % self._name_nostring_re, '',
                                self.en_name,
        ...
        # 自定义占位符
        self.customization = CustomizationMatcher().match(
            title=original_title) or None
        # 视频编码
  ...
  def __prepare_title(title):
    ...
    # 去掉大小
    title = re.sub(r'[0-9.]+\s*[MGT]i?B(?![A-Z]+)', "", title,
                   flags=re.IGNORECASE)
    # 将TVxx改为xx
    ...
    # 处理 Ver1.1a - 01 格式和类似格式
    title = re.sub(r'(\w+)?(\d+\.\d+[a-zA-Z]?)\s*-\s*(\d+)', r'S\2E\3', title)
    # 处理/分隔的中英文标题
    names = title.split("]")
