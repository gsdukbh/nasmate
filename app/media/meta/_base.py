...
class MetaBase(object):
  ...
  def get_title_string(self):
    if self.title:
      ...
      return "%s (%s)" % (
        self.get_name(), self.year) if self.year else self.get_name()
    else:
      return ""
  ...
  def get_overview_string(self, max_len=140):
    ...
    overview = (overview[:max_len] + placeholder) if len(
        overview) > max_len else overview
    return overview
  ...
  def get_season_list(self):
    if self.begin_season is None:
      ...
      return [season for season in
              range(self.begin_season, self.end_season + 1)]
    else:
  ...
  def get_episode_list(self):
    if self.begin_episode is None:
      ...
      return [episode for episode in
              range(self.begin_episode, self.end_episode + 1)]
    else:
  ...
  def get_episode_items(self):
    return "E%s" % "E".join(
        str(episode).rjust(2, '0') for episode in self.get_episode_list())
  ...
  def get_detail_url(self):
    if self.tmdb_id:
      if str(self.tmdb_id).startswith("DB:"):
        return "https://movie.douban.com/subject/%s" % str(
            self.tmdb_id).replace("DB:", "")
  ...
  def get_volume_factor_string(self):
    return self.get_free_string(self.upload_volume_factor,
  ...
  def get_free_string(upload_volume_factor, download_volume_factor):
    ...
    return free_strs.get(
        '%.1f %.1f' % (upload_volume_factor, download_volume_factor), "未知")
  ...
  def set_tmdb_info(self, info):
    ...
    self.tmdb_info = info
    self.vote_average = round(float(info.get('vote_average')), 1) if info.get(
        'vote_average') else 0
    self.overview = info.get('overview')
    self.original_language = info.get('original_language')
    self.networks = [network.get("name") for network in
                     info.get('networks') or []]
    if self.type == MediaType.MOVIE:
      ...
      self.runtime = info.get("episode_run_time")[0] if info.get(
          "episode_run_time") else None
  ...
  def init_subtitle(self, title_text):
    ...
    if re.search(r'[全第季集话話期]', title_text, re.IGNORECASE):
      # 第x季
      season_str = re.search(r'%s' % self._subtitle_season_re, title_text,
                             re.IGNORECASE)
      if season_str:
      ...
      # 第x集
      episode_str = re.search(r'%s' % self._subtitle_episode_re, title_text,
                              re.IGNORECASE)
      if episode_str:
        ...
        self._subtitle_flag = True
      # x集全
      episode_all_str = re.search(r'%s' % self._subtitle_episode_all_re,
        ...
        if episode_all and self.begin_episode is None:
          try:
            self.total_episodes = int(
                cn2an.cn2an(episode_all.strip(), mode='smart'))
          except Exception as err:
            ExceptionUtils.exception_traceback(err)
            return
          ...
          self._subtitle_flag = True
      # 全x季 x季全
      season_all_str = re.search(r"%s" % self._subtitle_season_all_re,
                                 title_text, re.IGNORECASE)
      if season_all_str:
        ...
        if season_all and self.begin_season is None and self.begin_episode is None:
          try:
            self.total_seasons = int(
                cn2an.cn2an(season_all.strip(), mode='smart'))
          except Exception as err:
            ExceptionUtils.exception_traceback(err)
            return
          self.begin_season = 1
